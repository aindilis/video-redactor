...
import pprint
import ast
sys.path.append('/var/lib/myfrdcsa/codebases/internal/unilang/clients/Python')
from frdcsa import unilang

fourcc = cv2.VideoWriter_fourcc(*'MJPG')
vid = cv2.VideoWriter("/home/andrewdo/output.avi",fourcc, 25, (1920,1080))
...
grab_res = (1920,1080)
output_window_res = (1920,1080)

def cap_loop_with_tesseract_api(cap):
	debug = 0
	try:
		...
		while True:
			t('grab')
			# mon = {'left': 2560, 'top': 0, 'width': grab_res[0], 'height': grab_res[1]}
			...
			#screenShot
			img0 = frame # np.array(screenShot)
			...
			uniquetexts = dict()
			for rect in rects:
				if not rect['text'] in uniquetexts:
					if len(rect['text']) > 0:
						if (not rect['text'] in redacted):
		                                	uniquetexts[rect['text']] = 1
		                                	texts.append(rect['text'])

			pprint.pprint(json.dumps(texts) + "\n")

			if len(texts) > 0:
				print("Query VideoRedactor\n")
	                        # /var/lib/myfrdcsa/codebases/minor/video-redactor/sys/unilang.py
				response = unilang.queryAgent('VideoRedactor',json.dumps(texts))
				root = ET.fromstring(response[0])
				result = ""
				for child in root:
					if child.tag == "data":
						p = re.compile(r".*?\'Result\' => (\'|\")([^\']*)(\'|\").*?",re.DOTALL)
						m = p.match(child.text)
						res = m.group(2)
						result = saxutils.unescape(res)
	
				# pprint.pprint("HELLO: " + result)
				redactions = json.loads(result)
				# pprint.pprint(redactions)
...
if __name__ == '__main__':
	cap = cv2.VideoCapture("/home/andrewdo/iem2-3.mp4")
	cap_loop_with_tesseract_api(cap)
